...
export class RoutingService implements OnDestroy {
  ...
  subRoutedPageEpisodes() {
    this.store.pipe(select(selectRoutedPageEpisodes), takeUntil(this.destroyed$)).subscribe((episodes: Episode[]) => {
      this.episodes = episodes;
    });
  }

  subSelectedEpisodes() {
    this.store.pipe(select(selectAggregateSelectedEpisodeGuids), takeUntil(this.destroyed$)).subscribe((guids: string[]) => {
      this.aggregateSelectedEpisodeGuids = guids;
    });
    this.store.pipe(select(selectSelectedEpisodeDropdays), takeUntil(this.destroyed$)).subscribe((episodes: EpisodeDropday[]) => {
    ...
    switch (newRouterParams.metricsType) {
      case METRICSTYPE_DOWNLOADS:
        if (
          ...
          this.isEpisodePageChanged(newRouterParams) ||
      ...
      case METRICSTYPE_DROPDAY:
        if (this.isPodcastChanged(newRouterParams) || this.isMetricsTypeChanged(newRouterParams)) {
          ...
          this.dropdayEpisodes &&
          this.dropdayEpisodes.length &&
      ...
      case METRICSTYPE_DEMOGRAPHICS:
        if (this.aggregateSelectedEpisodeGuids && this.aggregateSelectedEpisodeGuids.length > 0) {
          if (
            this.isMetricsTypeChanged(newRouterParams) ||
            this.isGroupChanged(newRouterParams) ||
            this.isBeginDateChanged(newRouterParams) ||
            this.isEndDateChanged(newRouterParams)
          ) {
          ...
          if (
            newRouterParams.group === GROUPTYPE_GEOCOUNTRY &&
            newRouterParams.filter &&
            (this.isFilterChanged(newRouterParams) ||
              this.isPodcastChanged(newRouterParams) ||
              this.isBeginDateChanged(newRouterParams) ||
              this.isEndDateChanged(newRouterParams))
          ) {
  ...
  checkAndGetDefaults(routerParams: RouterParams): RouterParams {
    ...
    if (
      routerParams.metricsType === METRICSTYPE_TRAFFICSOURCES &&
      (!routerParams.group ||
      ...
      routerParams.metricsType === METRICSTYPE_DEMOGRAPHICS &&
      (!routerParams.group ||
        routerParams.group === GROUPTYPE_AGENTOS ||
  ...
  isGroupChanged(newRouterParams: RouterParams): boolean {
    return newRouterParams && newRouterParams.group && (!this.routerParams || this.routerParams.group !== newRouterParams.group);
  }

  isFilterChanged(newRouterParams: RouterParams): boolean {
    return newRouterParams && newRouterParams.filter && (!this.routerParams || this.routerParams.filter !== newRouterParams.filter);
  }

  isPodcastChanged(newRouterParams: RouterParams): boolean {
    return (
      newRouterParams && newRouterParams.podcastId && (!this.routerParams || this.routerParams.podcastId !== newRouterParams.podcastId)
    );
  }
