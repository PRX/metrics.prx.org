  ...
  describe('unknown action', () => {
    ...
    it('should set loading on episode all time downloads load', () => {
      const newState = reducer(initialState,
        new ACTIONS.CastleEpisodeAllTimeDownloadsLoadAction({podcastId: episodes[0].podcastId, guid: episodes[0].guid}));
    ...
    it('should set error on failure', () => {
      const newState = reducer(initialState, new ACTIONS.CastleEpisodeAllTimeDownloadsFailureAction({
        podcastId: episodes[1].podcastId,
        guid: episodes[1].guid,
