...
describe('Episode Select Reducer', () => {
  ...
  it('should hold onto the last search term', () => {
    let newState = reducer(
      initialState,
      ACTIONS.CastleEpisodeSelectPageLoad({ podcastId: '70', page: 1, per: EPISODE_SELECT_PAGE_SIZE, search: 'cool stuff' })
    );
    expect(newState.search).toEqual('cool stuff');
    newState = reducer(
      newState,
      ACTIONS.CastleEpisodeSelectPageSuccess({ page: 1, per: EPISODE_SELECT_PAGE_SIZE, total: episodes.length, episodes })
    );
    expect(newState.search).toEqual('cool stuff');
  });
  ...
  it('saves selected episodes between searches', () => {
    let newState = reducer(
      initialState,
      ACTIONS.CastleEpisodeSelectPageLoad({ podcastId: '70', page: 1, per: EPISODE_SELECT_PAGE_SIZE, search: 'cool' })
    );
    newState = reducer(
      newState,
      ACTIONS.CastleEpisodeSelectPageSuccess({ page: 1, per: EPISODE_SELECT_PAGE_SIZE, total: episodes.length, episodes })
    ...
    newState = reducer(
      newState,
      ACTIONS.CastleEpisodeSelectPageLoad({ podcastId: '70', page: 1, per: EPISODE_SELECT_PAGE_SIZE, search: 'something else' })
    );
    newState = reducer(
      newState,
      ACTIONS.CastleEpisodeSelectPageSuccess({ page: 1, per: EPISODE_SELECT_PAGE_SIZE, total: episodes.length, episodes })
  ...
  it('should reset search term and selections when routing to a podcast', () => {
    const newState = reducer(initialState, ACTIONS.RoutePodcast({ podcastId: '70' }));
  ...
  it('should set loading on episode select page load', () => {
    const newState = reducer(
      initialState,
      ACTIONS.CastleEpisodeSelectPageLoad({ podcastId: '70', page: 1, per: EPISODE_SELECT_PAGE_SIZE })
    );
  ...
  it('should update an episode entity if it was already on the state', () => {
    ...
    // @ts-ignore using upsert adds 'id' property to entity, seems like ngrx/entity v6 gets rid of this
    expect(newState.entities[episodes[1].guid]).toEqual({ ...episodes[1] });
    newState = reducer(
      newState,
  ...
  it('should set error on failure', () => {
    const newState = reducer(initialState, ACTIONS.CastleEpisodeSelectPageFailure({ error: 'something went wrong' }));
  ...
  it('should have total episodes', () => {
    let newState = reducer(
      initialState,
      ACTIONS.CastleEpisodeSelectPageSuccess({ page: 2, per: EPISODE_SELECT_PAGE_SIZE, total: episodes.length, episodes })
    );
    expect(newState.total).toEqual(episodes.length);
    newState = reducer(newState, ACTIONS.CastleEpisodeSelectPageSuccess({ page: 1, per: EPISODE_SELECT_PAGE_SIZE, total: 25, episodes }));
  ...
  it('should update with toggled episodes', () => {
    ...
    let newState = reducer(
      initialState,
      ACTIONS.ChartToggleEpisode({ podcastId: episodes[0].podcastId, guid: episodes[0].guid, charted: true })
    );
    ...
    newState = reducer(newState, ACTIONS.ChartToggleEpisode({ podcastId: episodes[0].podcastId, guid: episodes[0].guid, charted: false }));
    expect(newState.downloadsSelected[episodes[0].podcastId]).toEqual(episodes.slice(1).map(e => e.guid));

    // add to selected
    newState = reducer(newState, ACTIONS.ChartToggleEpisode({ podcastId: episodes[0].podcastId, guid: 'jklmnop', charted: true }));
