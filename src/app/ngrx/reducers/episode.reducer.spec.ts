...
describe('Episode Reducer', () => {
  ...
  it('should set pagesLoading on episode page load', () => {
    const newState = reducer(initialState, ACTIONS.CastleEpisodePageLoad({ podcastId: '70', page: 1, per: EPISODE_PAGE_SIZE }));
  ...
  it('should set episode entities and pagesLoaded on episode page success', () => {
    const newState = reducer(
      initialState,
      ACTIONS.CastleEpisodePageSuccess({ page: 1, per: EPISODE_PAGE_SIZE, total: episodes.length, episodes })
    );
  ...
  it('should set error on failure', () => {
    const newState = reducer(initialState, ACTIONS.CastleEpisodePageFailure({ error: 'something went wrong' }));
  ...
  it('should update total episodes on page 1 or on initial load', () => {
    let newState = reducer(
      initialState,
      ACTIONS.CastleEpisodePageSuccess({ page: 2, per: EPISODE_PAGE_SIZE, total: episodes.length, episodes })
    );
    expect(newState.total).toEqual(episodes.length);
    newState = reducer(newState, ACTIONS.CastleEpisodePageSuccess({ page: 1, per: EPISODE_PAGE_SIZE, total: 25, episodes }));
    expect(newState.total).toEqual(25);
    newState = reducer(newState, ACTIONS.CastleEpisodePageSuccess({ page: 2, per: EPISODE_PAGE_SIZE, total: episodes.length, episodes }));
    expect(newState.total).toEqual(25);
  });
});
