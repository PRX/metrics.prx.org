import { ActionTypes, ActionWithPayload, All, CmsPodcastFeedAction, CmsEpisodeGuidAction } from '../actions/action.types';
import { PodcastModel } from '../model';

const initialState = [];
export function PodcastReducer(state: PodcastModel[] = initialState, action: ActionWithPayload<All>) {
  let podcastIdx: number, podcast: PodcastModel, newState: PodcastModel[];
  switch (action.type) {
    case ActionTypes.CMS_PODCAST_FEED:
      // instanceof because TypeScript
      if (action instanceof CmsPodcastFeedAction) {
    ...
    case ActionTypes.CMS_EPISODE_GUID:
      // instanceof because TypeScript
      if (action instanceof CmsEpisodeGuidAction) {
        podcastIdx = state.findIndex(p => p.seriesId === action.payload.podcast.seriesId);
        if (podcastIdx > -1) {
          podcast = Object.assign({}, state[podcastIdx]);
