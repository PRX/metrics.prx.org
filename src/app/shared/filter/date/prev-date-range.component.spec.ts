...
describe('PrevDateRangeComponent', () => {
  ...
  it('should go to prev range when PREV button is click', () => {
    ...
    comp.store.dispatch(new CastleFilterAction({filter: newFilter}));
    expect(comp.filter.when).toEqual(THIS_WEEK);
    comp.prev();
    expect(comp.filter.when).toEqual(LAST_WEEK);
  ...
  it('when PREVing from THIS_MONTH, should go to LAST_MONTH', () => {
    ...
    comp.store.dispatch(new CastleFilterAction({filter: newFilter}));
    expect(comp.filter.when).toEqual(THIS_MONTH);
    comp.prev();
    expect(comp.filter.when).toEqual(LAST_MONTH);
  ...
  it('when PREVing from THREE_MONTHS, should go to PRIOR_THREE_MONTHS', () => {
    ...
    comp.store.dispatch(new CastleFilterAction({filter: newFilter}));
    expect(comp.filter.when).toEqual(THREE_MONTHS);
    comp.prev();
    expect(comp.filter.when).toEqual(PRIOR_THREE_MONTHS);
